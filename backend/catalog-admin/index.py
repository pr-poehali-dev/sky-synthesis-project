...
def handler(event: dict, context) -> dict:
    ...
    if method == 'GET':
        cur.execute("SELECT id, title, description, category, image, downloads, rating, tag, file_url FROM catalog_items ORDER BY id DESC")
        rows = cur.fetchall()
        items = [
            {'id': r[0], 'title': r[1], 'description': r[2], 'category': r[3],
             'image': r[4], 'downloads': r[5], 'rating': float(r[6]), 'tag': r[7], 'file_url': r[8]}
            for r in rows
        ]
        cur.close()
        conn.close()
    ...
    if method == 'POST':
        ...
        cur.execute(
            "INSERT INTO catalog_items (title, description, category, image, downloads, rating, tag, file_url) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id",
